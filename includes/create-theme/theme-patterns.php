...
class CBT_Theme_Patterns {
	...
	public static function pattern_from_wp_block( $pattern_post, $options ) {
	...
	public static function replace_local_synced_pattern_references( $pattern ) {
		...
		// Replace references to the local unsynced patterns in the theme
	...
	public static function add_patterns_to_theme( $options = null ) {
		...
		if ( $pattern_query->have_posts() ) {
			...
			foreach ( $pattern_query->posts as $pattern ) {
				$pattern        = self::pattern_from_wp_block( $pattern, $options );
				self::add_pattern_to_theme( $pattern );

				if ( $pattern->is_synced === 'yes' ) {
					$synced_patterns[] = $pattern;
				}
			}

			// now replace all instances of synced blocks with pattern blocks
			// we wait uttil all patterns are added to the theme before doing this
			// because some patterns may reference other patterns
